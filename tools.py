...
class TextData:
	...
	# Wrappers for saving data frame to the RF 
    # object for when process() isn't used
	...
	# Another wrapper for the vectorization functions; 
    # optional, and will take a while
	def process(self, 
             df, 
             x_name, 
             y_name=None, 
	...
	def split(self, 
		...
		if method == 'train-test':
			...
			self.X_train, self.X_test, 
            self.y_train, self.y_test = split_by_var(self.X, self.y, data,
												split_var, test_val)
...
# Wrapper for inspect.getargspec; helps navigate my 
# less-than-ideal naming conventions
def get_args(foo):
...
def diagnostics(guesses, targets):
	out = pd.DataFrame(np.zeros([1, 5]), 
                    columns=['se', 'sp', 'ppv', 'f1', 'acc'])
